...
доступным окном, а HOME снаружи остаётся интерактивным. Поверх задачи Atlas рисует только
настраиваемое визуальное оформление и элементы управления.
...
- до пяти пресетов с точным launcher `ComponentName` и выбором из установленных приложений;
- пять стабильных launcher-слотов: каждый занятый слот получает отдельную обычную Activity-иконку
  без зависимости от поддержки pinned/dynamic shortcuts;
- постоянная launcher Activity «Atlas: Показать / закрыть» переключает последнее выбранное окно;
...
- для шапки, скругления и кнопок — разрешение «поверх других приложений»;
...
Команда показа сначала выводит HOME, а затем запускает freeform-задачу поверх него. После появления
окна повторно нажимать системную кнопку HOME не нужно: Android 11 может отправить в фон и саму
freeform-задачу. Обычный APK не может перехватить или отменить системное действие HOME.

Шапка и маска Atlas создаются уже на этапе запуска, до завершения опциональной ADB-проверки.
Прозрачные launcher/command Activity Atlas не считаются переходом в другое приложение и не скрывают
оформление. Если локальный ADB недоступен, повторный запуск существующей задачи не гарантирует
возврат bounds после ручного изменения размера: публичного API resize чужой task у Android нет.
...
5. Запускать занятые слоты через появившиеся обычные иконки «Atlas: Пресет 1…5».
...
Публичные командные intent принимает прозрачная `CommandActivity`. Она не принимает произвольный
package или shell-аргументы — только ID уже сохранённого пресета. Пять экспортированных launcher
aliases не принимают цель извне: каждый жёстко связан со своим локальным слотом.

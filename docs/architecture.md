...
предыдущее состояние вместо ложного скрытия шапки. Прозрачные `WindowToggleActivity`,
`PresetLauncherActivity` и `CommandActivity` также сохраняют состояние; полноценная
`MainActivity` настроек по-прежнему считается отдельным foreground-экраном. Provisional chrome
создаётся уже при `LAUNCHING` и откатывается к прежнему пресету либо удаляется при ошибке.
